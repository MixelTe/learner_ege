...
export const data: TestItem[] = [
	...
	new TestItemSelfCheck(25, DBc("Работа силы"), createFormulas("A = |&F|*|'d&r|*@{cos}'a [Дж]", "A_x = F_x * 'dx")),
	...
	new TestItemSelfCheck(94, DBc("ЭДС индукции в прямом проводнике длиной ").formula(createFormulas("l ")).text(", движущемся со скоростью ").formula(createFormulas("@{&{'v} (&{'v} '/ l )}")).text(" в однородном магнитном поле ").formula(createFormulas("B")), createFormulas("||'E_i| = Bl'v@{cos}'a", "\\'a - угол между вектором B и нормалью &n к плоскости, в которой лежат векторы &l и &{'v}", "\\B - магнитная индукция [1 Тл] (тесла)")),
